...
if not BACKUP_LOG.exists():
    try: BACKUP_LOG.touch(exist_ok=True)
    except: pass
if not SERVICE_LOG.exists():
    try: SERVICE_LOG.touch(exist_ok=True)
    except: pass
...
index_mode = os.environ.get('INDEX_MODE','memory')
worker = BackupWorker(task_queue, BACKUP_LOG, INDEX_DB, ALLOWED_ROOTS, ops_per_sec=default_rate, service_log=SERVICE_LOG, index_mode=index_mode)
worker.start()
        ...
        with os.scandir(p) as it:
            for entry in it:
                ...
                try:
                    ...
                    is_dir = False
                entries.append({'name': entry.name, 'path': str(Path(p)/entry.name), 'is_dir': is_dir})
                count += 1
    except Exception as e:
        try:
            for name in os.listdir(p)[:MAX_ENTRIES]:
                ...
                try:
                    ...
                    is_dir = False
                entries.append({'name': name, 'path': str(entry_path), 'is_dir': is_dir})
